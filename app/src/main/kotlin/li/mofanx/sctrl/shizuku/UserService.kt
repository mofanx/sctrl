...
class UserService(val context: Context) : IUserService.Stub() {
    ...
    override fun setDisplayPowerMode(turnOff: Boolean): Boolean {
        Log.d("UserService", "setDisplayPowerMode(turnOff=$turnOff)")
        return ScreenControlHelper.setDisplayPowerMode(turnOff)
    }

    override fun setStayAwake(enable: Boolean): Boolean {
        Log.d("UserService", "setStayAwake(enable=$enable)")
        return ScreenControlHelper.setStayAwake(enable)
    }
...
data class UserServiceWrapper(
    ...
    fun setDisplayPowerMode(turnOff: Boolean): Boolean = try {
        userService.setDisplayPowerMode(turnOff)
    } catch (e: Throwable) {
        e.printStackTrace()
        false
    }

    fun setStayAwake(enable: Boolean): Boolean = try {
        userService.setStayAwake(enable)
    } catch (e: Throwable) {
        e.printStackTrace()
        false
    }
