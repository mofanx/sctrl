...
import li.mofanx.sctrl.data.UserInfo
import li.mofanx.sctrl.data.otherUserMapFlow
import li.mofanx.sctrl.permission.allPermissionStates
import li.mofanx.sctrl.shizuku.currentUserId
...
private val filesDir: File by lazy {
    val markFile = app.filesDir.resolve(".sctrl")
    if (markFile.isFile) {
        app.filesDir
    } else {
        // fix #1333
...
fun buildLogFile(): File {
    val tempDir = createSctrlTempDir()
    ...
    tempDir.resolve("sctrl-${META.versionCode}-v${META.versionName}.json").also {
